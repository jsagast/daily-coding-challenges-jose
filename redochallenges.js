...
const computeRemainder=(numA,numB)=>{
...
const range=(a,b)=>{
    return a>b? 'first argument must be less than second': Array.from({ length:(b-a) }, (_, index) => index+a);
}
...
const charCount=(str)=>{
  ...
  return charArray.reduce(function(acc, character){
    if(acc[character]){
...
const formatWithPadding=(num1, character, num2)=>{
  if(num2>num1.toString().length){
    return (character.repeat(num2-num1.toString().length)+num1);
  } else {
...
//   // casing/spaces not included
//   const cleaned = str.toLowerCase().replace(/[\W]/g, '')
//   console.log(cleaned)

//   // char matches backward => forward
//   let left = 0;
...
//   // return true if while loop doesn't return false
//   return true;
// }
...
const hammingDistance=(str1, str2)=>{
    ...
    return str1.split('').reduce((count, char, index)=> char !== str2[index]? count+=1: count, 0)
...
//     if (highest === null || current.price > highest.price) {
//       return current;
//     }
//     return highest; 
//   }, null);
...
//         if (items[i].price > items[highestIdx].price) {
//             highestIdx = i
//         }
//     }
...
//     // new array that we're going to return
//     let flattened = [];
...
//     // primes start at 2
//     for (let i = 2; i<= num; i++) {
//         // another loop to check if num is divisible by i
//         while (num % i === 0) {
//             // if so, push into array
//             primes.push(i)
//             // now num is updating to num by dividing num with i
//             num = num / i;
//         }
//     }
...
const isPrime=(int)=>{
  ...
  if (int<=1 || int%2===0)return false;

  for(let i=3; i<=Math.sqrt(int); i+=2){// to skip even numbers and math.sqr to reduce iterations
    if (int%i===0){
      return false
    };
  };
...
//   const match = arr2.filter(element => arr1.includes(element));
//   return arr1.some(element => arr2.includes(element))? match: [];
// };

const intersection = (arr1, arr2) => {
// to avoid modifying arr 2
  const arr2Copy = [...arr2];

  // Filter arr1: only include element if it exists in arr2Copy
const match = arr1.filter(element => {
  const i = arr2Copy.indexOf(element);
  return i !== -1 && arr2Copy.splice(i, 1); //assigns to match
});
...
//   // storing all elements as key in storedValues
//   // keeping track of how many times that value
//   // appears in arr1
//   for (let i = 0; i < arr1.length; i++) {
//     if (arr1[i] in storedValues) {
//       storedValues[arr1[i]] += 1;
//     } else {
//       storedValues[arr1[i]] = 1;
//     }
//   }

//   // checking if arr2 to see which ones exist in storedValues
//   for (let i = 0; i < arr2.length; i++) {
//     if (arr2[i] in storedValues && storedValues[arr2[i]] > 0) {
//       output.push(arr2[i]);
//       storedValues[arr2[i]] -= 1;
//     }
//   }
...
const balancedBrackets=(str)=>{
  ...
  for(let i=0; i<str.length;i++){
    if(str[i]==='(' || str[i]==='{' || str[i] ==='['){
      ...
      const lastBracket=values.pop();
      if(lastBracket==='(' && str[i] !==')'|| lastBracket==='{'&& str[i] !=='}' || lastBracket ==='['&& str[i] !==']') return false
    };
...
//     if (!value1.includes(String.fromCharCode(value2))) {
//       return false;
//     }
//   }
//   return true;
...
const isWinningTicket = (arr) =>
  arr.every(([str, code]) =>
    str.includes(String.fromCharCode(code))// result of every method is boolean
);
...
const gridTrip = (arr, str) => {
  ...
  const instructions = str.match(/[UDLR]\d+/g);//d+ more than one number
  //instructions [U#, R#, l#, D#]
  //[d5,l15,u2]
  ...
  for (const instruction of instructions) {
    ...
    switch (direction) {
      case 'U':
        y += distance;
        break;// if don't use brake, it will follow
      case 'D':
        y -= distance;
        break;
      case 'R':
...
//       sumNumbers.push(array[i]+array[j]);
//     }
//   };
//   return sumNumbers.includes(num)
...
const totalTaskTime = (queue, threads) => {
  ...
  const cpus = Array.from({ length: threads }, () => 0);
  // cpus (3)= [0,0,0]

  for (let time of queue) {
    let minIndex = 0; // to compare and grab the lowest
    for (let i = 1; i < cpus.length; i++) {
      if (cpus[i] < cpus[minIndex]) {
        minIndex = i;
      }
    }
    cpus[minIndex] += time;
